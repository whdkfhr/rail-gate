...
| 항목 | 내용 |
|---|---|
| **I-12 는 여전히 운영에서 강제되지 않는다** | **E-A 종료 당시** — 이 Task 는 quota 범위가 가리킬 모델을 만들었을 뿐이고 테이블·저장소·서비스가 없었다.<br>**[2G-E-B2](TASK-002G-E-B2-sale-event-persistence.md) 완료 후 현재** — 범위를 표현하고 조회하는 테이블(V4·V5)과 저장소는 구현됐다. 그러나 `user_hold_quota` 와 상한 강제 로직, 애플리케이션 서비스가 없어 **I-12 자체는 아직 강제되지 않는다** |
| **DB 직접 SQL 의 소속 변경** | **E-A 종료 당시** — 도메인의 인스턴스 불변과 공개 재배정 API 부재뿐이었고, 같은 `TrainScheduleId` 의 영속 소속 갱신은 어디에서도 막히지 않았다.<br>**[2G-E-B2](TASK-002G-E-B2-sale-event-persistence.md) 완료 후 현재** — 저장소가 재배정 UPDATE 를 제공하지 않고(정상 애플리케이션 경로 차단), DB 의 FK 가 유효한 `sale_event` 참조를 보장한다. 트리거와 컬럼 권한 회수는 **미적용**이다. **즉 도메인과 저장소의 정상 애플리케이션 경로에서는 소속 재배정을 차단하지만, DB 직접 SQL 수준의 `sale_event_id` 변경은 아직 차단하지 못한다** (수단 비교는 [2G-E-B1](TASK-002G-E-B1-schema-decision.md) §9~§11) |
...
| **`SaleEventStatus` 와 선점 경로의 연결 없음** | 저장소의 조건부 전이는 구현됐지만([2G-E-B2](TASK-002G-E-B2-sale-event-persistence.md) §7), `isOpen()` 을 실제 선점 요청이 검사하도록 배선하는 것은 여전히 후속 Task 다 |
...
1. ~~**Task 2G-E-B — 운영 스키마와 저장소**~~ → 둘로 나뉘었다.
   ...
   - **2G-E-B2 (완료)** — [2G-E-B2](TASK-002G-E-B2-sale-event-persistence.md) : V4·V5 적용(V6 은 만들지 않았다),
     운영 저장소, 회차 전이의 조건부 UPDATE, 테스트 fixture 이행.
     트리거는 **적용하지 않기로** 판단했다 — 마이그레이션 계정 분리가 전제다
