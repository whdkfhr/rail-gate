...
>
> *(→ 그 미검증 위험은 [TASK-002F](TASK-002F-transaction-boundary.md)에서 실제로 재현됐고
> 트랜잭션 계약을 바꿔 해소했다. 아래 서술은 TASK-002C 시점 기준으로 유지한다.)*
...
### ★ 외부 트랜잭션 참여 시 동작 — 미검증 *(TASK-002C 시점)*

> **현재 상태.** 이 위험은 [TASK-002F](TASK-002F-transaction-boundary.md)에서 재현됐다.
> 외부 트랜잭션 참여 시 실제로 `UnexpectedRollbackException`이 발생했고,
> 저장소가 트랜잭션을 소유하지 않도록 계약을 바꿔 해소했다.
> 아래는 당시 미검증 상태의 기록이며 그대로 둔다.
